...
- Email verification uses SMTP settings from `.env`. When SMTP is not configured, registration returns `emailVerification.devVerificationUrl` so local development can still verify the account manually.
